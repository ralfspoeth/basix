...
public class Graph<T, L extends Comparable<? super L>> implements Function<T, Map<L, T>> {

    private Graph(Collection<T> nodeCollection, List<Edge<T, L>> edges) {
    ...
    public static <T, L extends Comparable<? super L>> Graph<T, L> usingLabelMap(Collection<T> elements, Function<T, Map<L, T>> successors) {
        ...
        return new Graph<>(elements, edges);
    }


    public static <T> Graph<T, EquiLabel<Void>> usingUnlabeledCollection(Collection<T> elements, Function<T, Collection<T>> successors) {
        return usingLabelMap(elements, e -> Map.of(EquiLabel.CONSTANT, e));
    }

    public static <T, L extends Comparable<? super L>> Graph<T, ComparableLabel<L>> usingComparableLabeledSuccessors(Collection<T> elements, Function<T, Map<L, T>> successors) {
